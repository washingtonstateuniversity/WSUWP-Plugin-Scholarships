...
import { useFetch } from "../../../assets/src/hooks";
...
const Edit = (props) => {
  ...
  const blockClass = "wsu-gutenberg-scholarships-list";
  const apiPath =
    "/wp-json/wsu-scholarships/v1/get-scholarships?postsPerPage=5";

  const domain =
    attributes.data_source === "custom"
      ? attributes.custom_data_source
      : WSUWP_SCHOLARSHIPS_PLUGIN_DATA.siteUrl;
  ...
  return (
    <>
      <InspectorControls>
        <PanelBody title="Scholarships List Settings" initialOpen={true}>
          <DataSource
            label="Data Source"
            help=""
      ...
      <div className={`${blockClass} ${className}`}>
        <div className={`${blockClass}__filters`}>
          <div className={`${blockClass}__default-filters`}>
            ...
            <div className={`${blockClass}__filter`}>G.P.A.</div>
            <div className={`${blockClass}__filter`}>
              - Citizenship -
              <span
                className={`${blockClass}__filter-icon dashicons dashicons-arrow-down-alt2`}
          ...
          <div className={`${blockClass}__filters-toggle`}>
            Show filters
            <span className={`${blockClass}__filters-toggle__icon`}>+</span>
          </div>
        </div>

        <div className={`${blockClass}__meta`}>
          <p className={`${blockClass}__meta-icon`}>
            <WSULogo className={`${blockClass}__coug-head`} />= scholarships
            awarded by Washington State University.
          </p>
          <p className={`${blockClass}__meta-count`}>
            {data && `${data.totalCount} scholarships found`}
          </p>
        </div>

        <div className={`${blockClass}__table-container`}>
          <table className={`${blockClass}__table wsu-table--style-striped`}>
            <thead>
              <tr>
                <th>
                  <span className={`${blockClass}__sort-control is-active asc`}>
                    Scholarship
                  </span>
                </th>
                <th>
                  <span className={`${blockClass}__sort-control`}>Amount</span>
                </th>
                <th>
                  <span className={`${blockClass}__sort-control`}>
                    Deadline
                  </span>
                </th>
                <th>
                  <span className="wsu-screen-reader-only">Application</span>
                </th>
              </tr>
            </thead>
            <tbody>
                    ...
                    <tr key={s.id}>
                      <td>
                        {s.data.includes("tag-wsu") && (
                          <WSULogo className={`${blockClass}__coug-head`} />
                        )}
                        <a
                          href={s.permalink}
                      ...
                      <td>
                        {s.applyLink && (
                          <a
                            className={`${blockClass}__apply-link`}
                            href={s.applyLink}
                          >
                            Apply
                          </a>
        ...
        {error && <p>Error: Failed to load scholarships</p>}
      </div>
    </>
  );
};

function WSULogo({ className }) {
  return (
    <svg
      className={`${className}`}
      role="img"
